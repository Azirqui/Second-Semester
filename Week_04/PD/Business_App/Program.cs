...
namespace Business_App
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            string path = "G:\\Nextcloud\\C#\\OOP Lab\\Week 05\\Self Work\\2022-CS-91_Business_App\\Credentails.txt";
            string admin_info_path = "G:\\Nextcloud\\C#\\OOP Lab\\Week 05\\Self Work\\2022-CS-91_Business_App\\Info_admin.txt";
            string user_info_path = "G:\\Nextcloud\\C#\\OOP Lab\\Week 05\\Self Work\\2022-CS-91_Business_App\\Info_user.txt";
            ...
            readuser_info(data_user,user_info_path);
            int choice = 0;
            if (check)
                    ...
                    if (user != null)
                    {
                        ...
                        else
                        {
                            while (close)
                            {
                                ...
                                if (choice == 1)
                                {
                                    bookTicket(data,data_user);
                                }
                                if (choice == 2)
                                {
                                    cancelBooking(data,data_user);
                                }
                                if (choice == 3)
                                {
                                    booked_Tickets(data_user);
                                }
                                if (choice == 4)
                                {
                                    print_list(data);
        ...
        // <----------------------- (User) User Menu  ---------------------->
        static int user_page()
        {
            int choice;
            Console.WriteLine("Enter Your Choice!");
            Console.WriteLine("1. Book a Ticket ");
            Console.WriteLine("2. Cancel Booking");
            Console.WriteLine("3. Current Booked Tickets");
            Console.WriteLine("4. Tickets Prices");
            Console.WriteLine("5. Available Discounts");
        ...
        // <----------------------- (User) Read User Info  ---------------------->
        static void readuser_info(List<Data_User> data_user, string user_info_path)
        {
            if (File.Exists(user_info_path))
            {
                StreamReader fileVariable = new StreamReader(user_info_path, true);
                ...
                while ((record = fileVariable.ReadLine()) != null)
                {
                    ...
                    float movie_price = float.Parse(parseData(record, 2));
                    string release_date = parseData(record, 3);
                    Data_User user_data = new Data_User(movie_name, movie_price, release_date);
                    data_user.Add(user_data);
                }
                fileVariable.Close();
            }
            else
        ...
        static void storeuser_info(List<Data_User> data_user, string user_info_path)
        {
            if (File.Exists(user_info_path))
            {

                StreamWriter fileVariable = new StreamWriter(user_info_path);
                for (int i = 0; i < data_user.Count; i++)
                {
                    fileVariable.WriteLine(data_user[i].user_movie_name + "," + data_user[i].user_movie_price + "," + data_user[i].user_movie_date);
                }
                fileVariable.Flush();
                fileVariable.Close();
            }
        ...
        // <----------------------- (User) Book Ticket  ---------------------->
        static void bookTicket(List<Data> data,List<Data_User> data_user)
        {
            print_list(data);
            Console.WriteLine(" Enter Movie Name: ");
            string movie_name = Console.ReadLine();
            ...
            Console.WriteLine(" Enter Releasing Date(dd/mm/yy): ");
            string release_date = Console.ReadLine();
            Data_User add_data = new Data_User(movie_name, movie_price, release_date);
            data_user.Add(add_data);
        }
        // <----------------------- (User) Cancel Booking  ---------------------->
        static void cancelBooking(List<Data> data,List<Data_User> data_user)
        {
            string remove_movie;
            bool flag;
            print_list(data);
            ...
            Console.WriteLine("Enter name of movie to be removed: ");
            remove_movie = Console.ReadLine();
            for (int s = 0; s < data_user.Count; s++)
            {
                if (data_user[s].user_movie_name == remove_movie)
                {
                    data_user.Remove(data_user[s]);
                    break;
                }
            }
            if (flag = false)
        ...
        // <----------------------- (User) Booked Tickets  ---------------------->
        static void booked_Tickets(List<Data_User> data_user)
        {
            Console.Clear();
            int x = 1;
            int y = 1;
            ...
            for (int i = 0; i < data_user.Count; i++)
            {
                ...
                Console.Write(i + 1 + ". ");
                Console.SetCursorPosition(x + 3, y);
                Console.Write(data_user[i].user_movie_name);
                Console.SetCursorPosition(x + 23, y);
                Console.Write(data_user[i].user_movie_price);
                Console.SetCursorPosition(x + 40, y);
                Console.Write(data_user[i].user_movie_date);
                Console.WriteLine();
                y++;
            }
        }

        // <----------------------- (Admin) Show List of Admin ---------------------->
        static void print_list(List<Data> data)
        {
            Console.Clear();
            int x = 1;
            int y = 1;
